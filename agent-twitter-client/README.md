...
async fn main() -> Result<()> {
    ...
    // Follow a user
    scraper.follow_user("RosieAISolana").await?;
    
    // Get user profile
    let profile = scraper.get_profile("RosieAISolana").await?;
    
    // Get user's followers
    let (followers, next_cursor) = scraper.get_followers("RosieAISolana", 20, None).await?;
